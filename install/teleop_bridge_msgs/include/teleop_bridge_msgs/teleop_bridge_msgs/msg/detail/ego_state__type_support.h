// generated from rosidl_generator_c/resource/idl__type_support.h.em
// with input from teleop_bridge_msgs:msg/EgoState.idl
// generated code does not contain a copyright notice

// IWYU pragma: private, include "teleop_bridge_msgs/msg/ego_state.h"


#ifndef TELEOP_BRIDGE_MSGS__MSG__DETAIL__EGO_STATE__TYPE_SUPPORT_H_
#define TELEOP_BRIDGE_MSGS__MSG__DETAIL__EGO_STATE__TYPE_SUPPORT_H_

#include "rosidl_typesupport_interface/macros.h"

#include "teleop_bridge_msgs/msg/rosidl_generator_c__visibility_control.h"

#ifdef __cplusplus
extern "C"
{
#endif

#include "rosidl_runtime_c/message_type_support_struct.h"

// Forward declare the get type support functions for this type.
ROSIDL_GENERATOR_C_PUBLIC_teleop_bridge_msgs
const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(
  rosidl_typesupport_c,
  teleop_bridge_msgs,
  msg,
  EgoState
)(void);

#ifdef __cplusplus
}
#endif

#endif  // TELEOP_BRIDGE_MSGS__MSG__DETAIL__EGO_STATE__TYPE_SUPPORT_H_
